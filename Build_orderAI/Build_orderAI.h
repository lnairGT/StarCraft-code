...
class Build_orderAI : public BWAPI::AIModule
{
public:
	...
	string obj_aff, obj_member;
	map<string, string> learned_object;

	void init_categories(string filename);
	string get_build_order(FILE *file);
	string get_membership(string filename, string member);
	string get_action(string filename, string category);
	string action_validate(string action, string parameter);
	string action_explore(string parameter);
...
struct map_unit : map<string, BWAPI::UnitType>
{
	map_unit()
	{
		...
		this->operator[]("Siege_Tank") = BWAPI::UnitTypes::Enum::Terran_Siege_Tank_Tank_Mode;
...
struct map_research : map<string, BWAPI::TechType>
{
	map_research()
	{
		...
		this->operator[]("Siege_Mode") = BWAPI::TechTypes::Enum::Tank_Siege_Mode;
...
struct map_upgrade : map<string, BWAPI::UpgradeType>
{
	map_upgrade()
	{
		...
		this->operator[]("Singularity_Charge") = BWAPI::UpgradeTypes::Enum::Singularity_Charge;
		...
		this->operator[]("Grooved_Spine") = BWAPI::UpgradeTypes::Enum::Grooved_Spines;
